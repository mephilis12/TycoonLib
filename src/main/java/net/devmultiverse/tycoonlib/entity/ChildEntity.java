...
import net.devmultiverse.tycoonlib.procedures.ChildOnEntityTickUpdateProcedure;
import net.devmultiverse.tycoonlib.procedures.ChildEntityDiesProcedure;
...
public class ChildEntity extends PathfinderMob implements GeoEntity {
	...
	public ChildEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TycoonlibModEntities.CHILD.get(), world);
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		ChildEntityDiesProcedure.execute(this.level(), this);
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.child.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.child.idle"));
		}
